<?php
    ...
    for($location = 0; $location < count($faqText); $location++)        #loop until $location has reached count of items in $locations array, then increase $location by 1
    {
        echo    "<div class='faq_popup_box_flex'>
                    <div data-target1='faq_text$location' class='faq_row toggle_button'>
                    <p>$faqText[$location]</p>    
                    <button type='button' class='faq_popup_box_collapsible'><img data-target2='faq_image$location' class='toggle_image' src='./img/arrow-dropdown.svg' id='faq_image$location' alt='&darr;'></button>
                    </div>
